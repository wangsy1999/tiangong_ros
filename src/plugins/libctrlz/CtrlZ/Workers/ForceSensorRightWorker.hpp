...
namespace z
{
    ...
    class ForceSensorRightWorker : public AbstractWorker<SchedulerType>
    {
        ///@brief 传感器数据必须是数值类型
    ...
    public:
         ...
         * @param root_cfg 配置文件
         */
        ForceSensorRightWorker(SchedulerType *scheduler, ForceSensorType ForceSensorInstancePtr, const nlohmann::json &root_cfg = nlohmann::json())
            : AbstractWorker<SchedulerType>(scheduler),
              ForceSensorInstance(ForceSensorInstancePtr)
        {
            this->PrintSplitLine();
            std::cout << "ForceSensorRightWorker" << std::endl;
        ...
        void TaskCycleBegin() override
        {
            ...
            this->Scheduler->template SetData<"ForceSensorRight">(data);
